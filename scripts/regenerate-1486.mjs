#!/usr/bin/env node
/**
 * Regenerate ONLY breed 1486 (Native American Indian Dog) avatar.
 * Uses the new Wisdom Panel reference image.
 */
...
async function main() {
  ...
  console.log(`Regenerating breed ${TARGET_BREED_ID} (Native American Indian Dog)...`)
  console.log("")
  
  const result = await generateOne(TARGET_BREED_ID, meta, model)
  
  const label = result.label || `Breed ${TARGET_BREED_ID}`
  if (result.ok) {
    const method = result.hasRef ? "using reference ref-1.png" : "prompt-only"
    console.log(`[GEN ] ${TARGET_BREED_ID} ${label} — ${method}`)
  } else {
    console.log(`[FAIL] ${TARGET_BREED_ID} ${label} — ${result.error || "unknown error"}`)
    process.exit(1)
  }
}
